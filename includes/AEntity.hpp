...
class AEntity
{
	...
	public:
		AEntity(int x, int y, int width, int height, const char *symbol, int health = -1, int colorPair = 0)
			: _x(x), _y(y), _symbol(symbol), _health(health), _colorPair(colorPair), _width(width), _height(height)
		{}
